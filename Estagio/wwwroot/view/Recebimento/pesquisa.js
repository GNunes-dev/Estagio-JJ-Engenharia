var pesquisa = {
    ...
    btnPesquisarOnClick: function () {
        ...
        document.getElementById("tbRecebimento").style.display = "block";
        ...
        fetch("/Recebimento/ObterTodos", config)
            ...
            .catch(function () {
                tbodyRecebimento.innerHTML = `<tr><td colspan="3">Deu erro...</td></tr>`
            })
    },
    ...
    quitar: function (id, valor) {
        $.ajax({
            type: 'POST',
            url: '/Recebimento/Quitar?id=' + id + '&valor=' + valor,
            contentType: 'application/json',
            success: function (res) {
                if (res.operacao) {
                    alert("Parcela quitada com sucesso");
                    var tableT = $('#tabelaRecebimento').DataTable();
    ...
    alterartabela: function () {
        var table = $('#tabelaRecebimento').DataTable();
        ...
        if (nValor <= 0) {
            ...
            $.ajax({
                type: 'POST',
                url: '/Recebimento/QuitarParcial?valorparcial=' + parc + '&id=' + id,
                contentType: 'application/json',
                success: function (res) {
                    if (res.operacao) {
